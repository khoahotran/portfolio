...
Snapshot at time of writing (2026-09-09, updated after Phase 8's tenth item):
62 articles across 6 collections, 29 interactive labs, 5 real Docker benchmark harnesses,
Phases 5-8 in progress (Phase 8 has shipped ten items — consistent hashing, the idempotency-key
store, vector clocks, CRDTs, Bloom filters, Merkle trees, Raft, Two-Phase Commit vs. Saga,
HyperLogLog, and LSM Trees — see `.ai/phases/phase-8.md` §8.1-8.10). One item (§5.2, flagship
deepening) genuinely blocked rather than deferred by choice — see Track A. Track B is empty — see
the note below it.

**Raft (§8.7), Two-Phase Commit vs. Saga (§8.8), HyperLogLog (§8.9), and LSM Trees (§8.10)** weren't
picked from a pre-populated Track B list — each time, asked to continue with both tracks empty, a
live scan (lab registry + `grep` across `content/`) surfaced fresh candidates presented directly,
and Khoa picked one. §8.7's scan surfaced Two-Phase Commit vs. Saga and HyperLogLog as the two
not-picked; §8.8's surfaced HyperLogLog again and Skip List; §8.9's surfaced LSM Tree and Skip List
(HyperLogLog itself was finally picked, after surfacing twice unpicked); §8.10's surfaced Skip List
(its third unpicked appearance) and MVCC & Write Skew. None of these rounds' leftovers were added to
Track B below; if they're still relevant next time Track B needs repopulating, they'd need
re-confirming as unshipped, not assumed from this note.
...
| `search-index.json` scale (§5.7) | Re-measured 2026-09-09: 320 KB / 62 docs (up from 187 KB / 38 docs) — growing, but still well under the trigger | Corpus crosses ~100 articles or the file crosses ~500 KB |
